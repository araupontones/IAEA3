...
wide <- raw_data %>%
  ...
  mutate(times_submitted = n(),
         int = row_number()) %>%
  relocate(country, name, answered, times_submitted,  int) %>%
  .[,1:7] %>%
  pivot_wider(id_cols = c(country, name, times_submitted),
...
openxlsx::saveWorkbook(wb, 'data/7.NLO/4.Check_unanswered_Part_II/check_duplicates.xlsx', overwrite = T)
