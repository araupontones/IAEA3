...
append_themes <- lapply(themes, function(t){
  ...
  r <- raw_data %>%
    ...
    keep_section(t, 'visibility', sufix) %>%
...
sort(unique(append_themes$value))

#clean relevance ---------------------------------------------------------------


  categories <- c("Not visible", "Low", "Moderately","Highly")


clean_visibility<- append_themes %>%
  mutate(level = str_extract(value, '[0-9]'),
         likert = case_when(level == 0 ~ categories[1],
                            level == 1 ~ categories[2],
                            level == 2 ~ categories[3],
                            level %in% c(3,4) ~ categories[4],
                            T ~ NA_character_
                            ),
         likert = factor(likert,
                         level = categories,
                         ordered = T)) %>%
  select(-c(value, level))
    
unique(clean_visibility$likert)     

#export ------------------------------------------------------------------------
export(clean_visibility, 'data/7.NLO/2.raw_formatted/Part_1_visibility.rds')
