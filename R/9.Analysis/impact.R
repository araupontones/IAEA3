...
cps <- import('data/6.data-collection/4.clean/cps.rds')

categories_i <- import('questionnaires/categories/impacts.xlsx') %>%
  rename(id = value,
         impact = title) %>%
...
impact <- cps %>%
  select(region,theme, starts_with('imp_opinion')) %>%
  mutate_all(as.character) %>%
  pivot_longer(-c(theme,region),
               names_to = 'id') %>%
  filter(!is.na(value)) %>%
  mutate(id = str_remove(id, "imp_opinion__"),
         value = as.numeric(value)) %>%
  left_join(categories_i) %>%
  select(-id) %>%
  group_by(region, theme, impact) %>%
  mutate(total= n()) %>%
  ungroup() %>%
  group_by(region, theme, total, impact) %>%
...
export(impact, 'data/11.powerbi/impacts_cp.csv')
